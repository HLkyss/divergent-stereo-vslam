...
//        cameraposevisual_.setScale(0.5);//相机可视化放大
        ...
        pub_point_cloud_l_to_r_ = n.advertise<pcl::PointCloud<pcl::PointXYZRGB>>("point_cloud_l_to_r", 1000);
        pub_point_cloud_r_to_l_ = n.advertise<pcl::PointCloud<pcl::PointXYZRGB>>("point_cloud_r_to_l", 1000);
...
*/
    ...
    void pubPointCloud_l_to_r(pcl::PointCloud<pcl::PointXYZRGB>::Ptr pcloud, const double time)
    {
        if( pub_point_cloud_l_to_r_.getNumSubscribers() == 0 ) {
            return;
        }
        ...
        pcloud->header = pcl_conversions::toPCL(header);
        pub_point_cloud_l_to_r_.publish(pcloud);
    }
    void pubPointCloud_r_to_l(pcl::PointCloud<pcl::PointXYZRGB>::Ptr pcloud, const double time)
    {
        if( pub_point_cloud_r_to_l_.getNumSubscribers() == 0 ) {
            return;
        }
        ...
        pcloud->header = pcl_conversions::toPCL(header);
        pub_point_cloud_r_to_l_.publish(pcloud);
    }
    ...
    ros::Publisher pub_point_cloud_l_to_r_, pub_point_cloud_r_to_l_;
